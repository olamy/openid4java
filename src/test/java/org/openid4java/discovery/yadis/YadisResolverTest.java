...
public class YadisResolverTest extends TestCase
{
    ...
    public void setUp() throws Exception
    {
        ...
        options.setSocketTimeout(100000);
        _resolver = new YadisResolver(new HttpFetcherFactory().createFetcher(options));

        _server = new Server(0);

        YadisTestServlet yadisTestServlet = new YadisTestServlet();

        ServletContextHandler context = new ServletContextHandler(ServletContextHandler.SESSIONS);
        context.addServlet(new ServletHolder(yadisTestServlet), "/*");

        _server.setHandler(context);

        _server.start();

        _servletPort = ((ServerConnector) _server.getConnectors()[0]).getLocalPort();
        yadisTestServlet.port = _servletPort;
